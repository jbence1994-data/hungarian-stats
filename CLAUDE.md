...
Vite + React 19 + TypeScript SPA. `src/main.tsx` mounts `<App />` into `#root` under `StrictMode`, wrapped in `QueryClientProvider` → `ThemeProvider` → `BrowserRouter`. `src/App.tsx` declares the routes: a `Layout` route (Navbar + `<Outlet>`) wraps the pages in `src/pages` — Overview (`/`) and Inflation Rate (`/inflation-rate`, under the Economy nav dropdown).
